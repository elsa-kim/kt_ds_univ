 ...
 * 
 * SqlSessionDaoSupport : MyBatis가 Database에 접속을 해서 데이터 제어하는 역할
 * 						- Insert, Update, Select, Delete
 * 
 * @Repository : Bean Container에 적재를 시키기 위한 힌트(데이터 접근 제어)
 * @Controller : Spring이 클래스를 객체화시켜서 Bean Container에 적재
 */
@Repository
public class BoardDaoImpl extends SqlSessionDaoSupport implements BoardDao{
	 ...
	 * DI(Depensency Injection)
	 * Bean Container에 보관되어있는 객체를 필요한 곳에 자동 주입하는 기술
	 * - 실행하는 방법
	 * - 1. 생성자를 이용하는 방법
	 * - 2. Setter를 이용하는 방법 --> @Autowired -> DI 필요한 애구나 -> 스프링이 파라미터 타입의 객체 전달해줌
	 * - 3. 멤버변수를 이용하는 방법
	 */
	@Autowired
	@Override
	...
	public int getBoardAllCount() {
		...
		// 서버 : Database 서버
		// 클라이언트 : 스프링 애플리케이션
		// sqlSession : sql 서버와 클라이언트가 연결되어있는 정보
		// sqlSession을 받아오는 명령어 getSqlSession() --> sqlSessionTemplate 반환
	...
	public int insertNewBoard(BoardVO boardVO) {
		/*
		 * MyBatis를 호출해서 쿼리가 실행될 수 있도록 코드 작성.
		 * 코드에 파라미터가 필요한 경우엔 쿼리셀렉터 이후에 파라미터를 전달.
