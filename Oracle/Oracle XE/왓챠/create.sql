...
COMMENT ON TABLE PRODUCING IS '영화를 제작한 감독의 정보';

COMMENT ON COLUMN PRODUCING.PRODUCING_ID IS '제작의 고유 번호';

COMMENT ON COLUMN PRODUCING.DIRECTOR_ID IS '영화 감독의 고유 번호';

COMMENT ON COLUMN PRODUCING.MOVIE_ID IS '영화의 고유 번호';

CREATE UNIQUE INDEX PK_PRODUCING
	ON PRODUCING (
		PRODUCING_ID ASC
	)
	STORAGE (
...
ALTER INDEX PK_PRODUCING NOMONITORING USAGE;
...
COMMENT ON TABLE FOLLOWERS IS '회원의 팔로우 정보';

COMMENT ON COLUMN FOLLOWERS.FOLLOW_ID IS '팔로우의 고유 번호';

COMMENT ON COLUMN FOLLOWERS.USER_ID IS '회원의 아이디';

COMMENT ON COLUMN FOLLOWERS.FOLLOW_USER_ID IS '팔로우 대상 회원의 아이디';
...
ALTER INDEX PK_FOLLOWERS NOMONITORING USAGE;
...
COMMENT ON COLUMN DIRECTORS.PROFILE IS '감독의 프로필 사진 URL';
...
COMMENT ON COLUMN RATINGS.RATING_ID IS '평점의 고유번호';
...
COMMENT ON COLUMN ACTORS.PROFILE IS '배우의 프로필 사진 URL';
...
COMMENT ON COLUMN USERS.BACKGROUND IS '마이페이지에서 사용될 배경사진의 URL';

COMMENT ON COLUMN USERS.PROFILE IS '프로필 사진의 URL';
...
ALTER TABLE PRODUCING
	ADD
		CONSTRAINT FK_MOVIES_TO_PRODUCING
		FOREIGN KEY (
			MOVIE_ID
		)
		REFERENCES MOVIES (
...
ALTER TABLE PRODUCING
	ADD
		CONSTRAINT FK_DIRECTORS_TO_PRODUCING
		FOREIGN KEY (
			DIRECTOR_ID
		)
...
ALTER TABLE FOLLOWERS
	ADD
		CONSTRAINT FK_USERS_TO_FOLLOWERS
		FOREIGN KEY (
			USER_ID
		)
		REFERENCES USERS (
...
ALTER TABLE FOLLOWERS
	ADD
		CONSTRAINT FK_USERS_TO_FOLLOWERS2
		FOREIGN KEY (
			FOLLOW_USER_ID
		)
		REFERENCES USERS (
