...
-- MANAGER_ID가 NULL인 사원의 모든 부하직원을 조회한다
 SELECT LEVEL 
      , EMPLOYEE_ID
      , FIRST_NAME || ' ' || LAST_NAME AS NAME
      , MANAGER_ID
...
/*
 SELECT LEVEL 
  ...
  INNER JOIN DEPARTMENTS D
     ON D.DEPARTMENT_ID = E.DEPARTMENT_ID 
  WHERE D.DEPARTMENT_ID = 60 -- START WITH CONNECT BY PRIOR와 WHERE 함께 있을 경우 
  							 -- START WITH CONNECT BY PRIOR가 먼저 수행됨
  START WITH E.EMPLOYEE_ID = D.MANAGER_ID 
...
SELECT LEVEL
  ...
  FROM (SELECT E.EMPLOYEE_ID
		 ...
		 INNER JOIN DEPARTMENTS D
		    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
		 WHERE D.DEPARTMENT_ID = 60) EMP_DEPT
  START WITH EMP_DEPT.EMPLOYEE_ID = EMP_DEPT.DEPT_MANAGER_ID
CONNECT BY PRIOR EMP_DEPT.EMPLOYEE_ID = EMP_DEPT.EMP_MANAGER_ID
;
...
SELECT *
 ...
 WHERE EMPLOYEE_ID NOT IN (SELECT MANAGER_ID 
						 	 FROM EMPLOYEES 
						 	WHERE MANAGER_ID IS NOT NULL) -- NULL 있으면 NOT NULL 안됨
;	

SELECT *
  FROM EMPLOYEES
 WHERE EMPLOYEE_ID IN (SELECT MAN.EMPLOYEE_ID
 						 FROM EMPLOYEES MAN
